...
3. **Warn, don't block:** a batch with many keywords is high-volume — print ONE short warning that this can
   get the IP temporarily rate-limited and suggest proxies for large/repeated runs, then **proceed**.
4. POST a single job with the full `keywords` array + required fields (`max_time` seconds, string `lat`/`lon`).
   Use a generous `max_time` (e.g. `900`) since many keywords take longer. Poll in the BACKGROUND, download.
5. Present results grouped/clean and save the full file to disk.

Tip: `scripts/scrape.py --keywords-file <file> --city "<City, ST>"` already does all of this.
